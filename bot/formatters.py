def format_portfolio_message(holdings: list) -> str:
    ...
    msg += f"*Unrealized P&L:* ₹{pnl:,.2f} ({pnl_pct:.2f}%)\n\n"
    ...
    for h in sorted_h:
        ...
        msg += f"- {h.get('stock_code')}: {h.get('quantity')} shs @ ₹{h.get('average_price', 0):.2f} (Val: ₹{val:,.2f})\n"
    return msg
